...
$global:serviceInstances = Get-SPServiceInstance -All
...
function GetServersInFarm()
{
    ...
    foreach($svr in $global:servers | Select DisplayName, Id, Status, Role)
    {
...
function GetServiceInstances()
{
    ...
    "#########################################################################################"
    ""
     $serviceInstances = $global:serviceInstances | where{$_.Status -ne "Disabled"}
...
function GetSSARelatedTimerJobs
{
    ...
    "  SSA Related Timer Jobs "
    "#########################################################################################"
    $ssaJobs = $global:ssa.Id
    Get-SPTimerJob | ?{$_.Name -match $ssaJobs} | Select Name
    ""
    $tjText = @"
     - SSAs should have several timer jobs associated with them
      -- SP 2013 should have 8 timer jobs
      -- SP 2016 should have 10 timer jobs
      -- SP 2019 should have 12 jobs
